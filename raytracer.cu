...
// normal version of the ray tracer
...
int main(void) {
	...
	// allocate memory on the GPU for the output image (for normal and read-only versions of the code)
    ...
    //copy to constant memory implementation
	cudaMemcpyToSymbol(d_const_s, h_s, spheres_size);
	//copy to device memory (normal and read-only implementations)
	...
	// generate a image from the sphere data (normal version)
